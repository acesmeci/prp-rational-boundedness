...
def _sweep_one_network(args_tuple):
    ...
    results_ba = sweep_soa(wrapper, gen_dep, soa, n_trials_per_soa=trials_per_soa,
                           persistence=persistence, dt_lca=dt_lca, t0=t0, ITI=ITI,
                           z_task2_fixed=z_A, optimize_onset=optimize_onset)
    results_ca = sweep_soa(wrapper, gen_ind, soa, n_trials_per_soa=trials_per_soa,
                           persistence=persistence, dt_lca=dt_lca, t0=t0, ITI=ITI,
                           z_task2_fixed=z_A, optimize_onset=optimize_onset)
...
def main():
    ...
    ap.add_argument("--out_png", type=str, default="prp_pashler_match.png")
    
    # Pashler Overlays
    ap.add_argument("--add_pashler", action="store_true", default=True)
    ...
    # Define target Pashler SOAs (ms)
    target_soa_ms = np.array([50, 150, 300, 900], dtype=float)
    
    # Convert ms to internal simulation steps
    # Based on the user's logic: ms = steps * dt_lca * 500
    soa_steps = [int(round(ms / (args.dt_lca * 500.0))) for ms in target_soa_ms]
    print(f"Targeting SOAs: {target_soa_ms} ms -> Sim Steps: {soa_steps}")
    ...
    job_tuples = [(idx, str(ckpt_dir), args.z_task, soa_steps, args.trials_per_soa,
    ...
    dep_curves, ind_curves = [], []
    for idx, z_A, dep_rt2, ind_rt2 in sorted(results, key=lambda r: r[0]):
        if z_A is not None:
            dep_curves.append(dep_rt2); ind_curves.append(ind_rt2)

    dep_mean, ind_mean = nanmean(dep_curves), nanmean(ind_curves)
    dep_se, ind_se = nanse(dep_curves), nanse(ind_curves)

    # Scaling for plot
    soa_plot_ms = np.array(soa_steps) * args.dt_lca * 500.0
    dep_mean_ms, ind_mean_ms = dep_mean * 500.0, ind_mean * 500.0
    dep_se_ms, ind_se_ms = dep_se * 500.0, ind_se * 500.0

    plt.figure(figsize=(8, 5))
    
    # Plot Simulation
    plt.plot(soa_plot_ms, dep_mean_ms, "x--", color="#1f77b4", label="Simulation B→A")
    plt.fill_between(soa_plot_ms, dep_mean_ms - dep_se_ms, dep_mean_ms + dep_se_ms, color="#1f77b4", alpha=0.15)
    
    # Plot Pashler (1994) Figure 1
    if args.add_pashler:
        p_soa = np.array([50, 150, 300, 900], dtype=float)
        p_rt2 = np.array([700, 600, 525, 500], dtype=float)
        
        if args.pashler_mode == "relative":
            p_soa = p_soa - p_soa[0] + soa_plot_ms[0]
        if args.align_pashler_rt:
            p_rt2 = p_rt2 - p_rt2[0] + dep_mean_ms[0]
            
        plt.plot(p_soa, p_rt2, "ko-", linewidth=2.5, alpha=0.7, label=f"Pashler 1994 Fig 1 ({args.pashler_mode})")

    plt.xlabel("SOA (ms)")
    plt.ylabel("RT2 (ms)")
    plt.title(f"Pashler Match | p={args.persistence}")
    plt.legend()
